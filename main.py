...
class AllSprites(pygame.sprite.Group):
	def __init__(self):
		...
		# import
		self.fg_sky = pygame.image.load('c1 - setup/graphics/sky/fg_sky.png').convert_alpha()
		self.bg_sky = pygame.image.load('c1 - setup/graphics/sky/bg_sky.png').convert_alpha()
		tmx_map = load_pygame('c1 - setup/data/map.tmx')
...
class Main:
	def __init__(self):
		...
		# bullet images
		self.bullet_surf = pygame.image.load('c1 - setup/graphics/bullet.png').convert_alpha()
		self.fire_surfs = [
			pygame.image.load('c1 - setup/graphics/fire/0.png').convert_alpha(),
			pygame.image.load('c1 - setup/graphics/fire/1.png').convert_alpha()]

		# music
		self.music = pygame.mixer.Sound('c1 - setup/audio/music.wav')
	...
	def setup(self):
		...
		for obj in tmx_map.get_layer_by_name('Entities'):
			if obj.name == 'Player':		
				self.player = Player(
					...
					path = 'c1 - setup/graphics/player', 
					collision_sprites = self.collision_sprites, 
					shoot = self.shoot) 
			if obj.name == 'Enemy':
				Enemy(
					pos = (obj.x, obj.y), 
					path = 'c1 - setup/graphics/enemies/standard', 
					groups = [self.all_sprites, self.vulnerable_sprites], 
					shoot = self.shoot, 
					player = self.player, 
					collision_sprites = self.collision_sprites)

		self.platform_bordeer_rects = []
		for obj in tmx_map.get_layer_by_name('Platforms'):
			if obj.name == 'Platform':
				...
				self.platform_bordeer_rects.append(border_rect)

	def platform_collisions(self):
		for platform in self.platform_sprites.sprites():
			for border in self.platform_bordeer_rects:
